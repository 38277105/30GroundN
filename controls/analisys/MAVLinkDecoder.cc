...
#include "../calibration/QGC.h"
//#include <common/mavlink.h>
#include "connections/CommonStru.h"
MAVLinkDecoder::MAVLinkDecoder(Vehicle* vobj,QObject *parent) :
    QThread()
{
    Q_UNUSED(parent);
    ...
    // update by xingjiwei
    //m_vehicle=  VehicleManager::getInstance()->getActiveVehicle();

    qDebug()<<"I'm plane,please call me multicopter"<<m_vehicle->getName()<<"----------------------droneyee ground...";
    if(m_vehicle!=NULL)
    {
       connect(&m_vehicle->mavLinkMessageInterface, SIGNAL(emit_mavlinkMessage(mavlink_message_t)), this, SLOT(receiveMessage(mavlink_message_t)));//update by xingjiwei

      qDebug()<<"I'm plane,I have send data to droneyee ground for analasysis........";
    }
...
void MAVLinkDecoder::receiveMessage(mavlink_message_t message)
{
    ...
    uint8_t msgid = message.msgid;
     ...
     //qDebug()<<"receive message....>>>"<<msgid<<"decoder......";
