...
#include "../controls/calibration/QGC.h"
...
void LinechartWidget::createLayout()
{
    ...
    activePlot = new LinechartPlot(this);
...
{
    qDebug()<<"appendData"<<curve<<".............."<<variant;
    ...
    if (isVisible())
    {
      ...
      //  qDebug()<<"label:"<<label;
        if(!label)
        {
            ...
            addCurve(curve, unit);
          //   qDebug()<<"addCurve(curve, unit):"<<curve<<":::"<<unit;
        }
...
//    tr("Log Files (*.log)"),
//    "log"); // Default type
    QString fileName="data/log001.txt";
...
//        bool fill = (button == QMessageBox::Yes);
        ...
        compressor->startCompression(true);//xingjiwei revise
    }
    logButton->setText(tr("Start logging"));
