...
bool Bootloader::_binProgram(QSerialPort* port, const FirmwareImage* image)
{
    ...
    uint8_t imageBuf[PROG_MULTI_MAX+8];
    ...
    while (bytesSent < imageSize) {
        ...
        if (bytesToSend > PROG_MULTI_MAX) {
            bytesToSend = PROG_MULTI_MAX;
        }
        
        Q_ASSERT((bytesToSend % 4) == 0);

        int bytesRead = firmwareFile.read((char *)(imageBuf+8), bytesToSend);
        ...
        for(int i=0;i<10;i++)
        {
            ...
            {
                int packlen=bytesToSend+8;
                if (_write(port, (uint8_t)packlen))
                {
                    int dlen=packlen;
                    for(int i=0;i<4;i++)
                        imageBuf[i+4]=(uint8_t)((bytesSent>>(i*8) & 0xFF));
                    uint32_t nCrc =0;// crc32((uint8_t *)(imageBuf+4), dlen-4, 0);
                    for(int i=0;i<(dlen-4)/4;i++)
                        nCrc=crc32((uint8_t *)(imageBuf+4+i*4), 4, nCrc);

                    for(int i=0;i<4;i++)
                        imageBuf[i]=(uint8_t)((nCrc>>(i*8) & 0xFF));

                    if (_write(port, imageBuf, dlen)) {
                        if (_write(port, PROTO_EOC)) {
                            port->flush();

//                            quint32 flashCRC;
//                            _read(port, (uint8_t*)&flashCRC, sizeof(flashCRC), _verifyTimeout);
//                            qDebug()<<flashCRC;
                            if (_getCommandResponse(port))
                            {
                                failed=false;
        ...
        _imageCRC = crc32((uint8_t *)(imageBuf+8), bytesToSend, _imageCRC);
