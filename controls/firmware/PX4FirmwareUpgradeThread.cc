    ...
    _findBoardFirstAttempt(true)
{
    ...
    connect(_controller, &PX4FirmwareUpgradeThreadController::_openBootloader,              this, &PX4FirmwareUpgradeThreadWorker::_openBootloader);
...
void PX4FirmwareUpgradeThreadWorker::_3drRadioForceBootloader(const QGCSerialPortInfo& portInfo)
{
    ...
    if (_findBootloader(portInfo, true , false )) {
        return;
    }
    ...
    _findBootloader(portInfo, true , true );
}
...
bool PX4FirmwareUpgradeThreadWorker::_openBootloader(QString portName,bool isUSB)
{
    //qDebug << "_findBootloader" << portInfo.systemLocation();
    ...
    {
        if (_bootloader->sync(_bootloaderPort)) {
            bool success = _bootloader->getPX4BoardInfo(_bootloaderPort, bootloaderVersion, boardID, flashSize);
...
void PX4FirmwareUpgradeThreadController::openBootloader(const QString &portName,bool isUSB)
{
    _portName = portName;
    emit _openBootloader(portName,isUSB);
}
