...
void QGCRadioChannelDisplay::paintEvent(QPaintEvent *event)
{
    ...
    if (m_orientation == Qt::Vertical)
    {
        ...
        float val = (float)m_value/(float)(m_max - m_min);
//        QString valStr = QString::number(m_value);
        ...
        if (!m_showMinMax) {
            ...
            painter.drawRect(1,yVal,width()-3,((height()-2) - yVal));
        }
        else {
            //draw the value
            ...
            QString minstr = "-1"/*QString::number(m_min)*/;
            ...
            QString maxstr = "1"/*QString::number(m_max)*/;
    ...
    else //horizontal orientation
    {
        ...
        if (!m_showMinMax) {
            //draw just the value
            painter.drawRect(1,1,(width()-3) * ((float)(curVal-m_min) / (m_max-m_min)),(height()-3) );
        }
        else {
            //draw the value
            painter.drawRect(1,1,(width()-3) * ((float)curVal / 3000.0),(height()-3));

             //draw the min and max bars
            painter.setBrush(Qt::NoBrush);
            painter.setPen(minMaxColor);
            painter.drawRect(width() * ((float)m_min / 3000.0),2,((width()-1) * ((float)m_max / 3000.0)) - (width() * ((float)m_min / 3000.0)),(height()-5));
