...
static void sphereError(const real_1d_array &xi, real_1d_array &fi, void *obj)
{
...
bool Vector3d::setToLeastSquaresSphericalCenter(const QVector<Vector3d> &pointsOnSphere)
{
    ...
    minlmstate state;
    minlmcreatev(pointsOnSphere.count(), x, 100.0f,  state);
    ...
    // optimize it!
    minlmoptimize(state, &sphereError, NULL, (void *)&pointsOnSphere);
