    ...
    m_isMovable(true)
{
    ...
    connect(m_mousetimer, SIGNAL(timeout()), this, SLOT(NewMouseClick()));
...
WayPointItem::WayPointItem(internals::PointLatLng const& coord, double const& altitude, MapGraphicItem* map, const WayPointType& type, const bool& isShowNum):
    map(map),
    autoreachedEnabled(true),
    text(NULL),
    textBG(NULL),
    ...
    m_isMovable(true)
{
    if(type == NORMARLPOINT){
        picture.load(QString::fromUtf8(":/markers/images/bigMarkerGreen.png"));
        number=WayPointItem::snumber;
        ++WayPointItem::snumber;
    }else if(type == BOUNDARYPOINT){
        picture.load(QString::fromUtf8(":/markers/images/marker.png"));
    }
    this->setFlag(QGraphicsItem::ItemIsMovable,true);
    this->setFlag(QGraphicsItem::ItemIgnoresTransformations,true);
...
void WayPointItem::SetMoavle(const bool& isMovale){
    m_isMovable = isMovale;
}
...
void WayPointItem::RefreshWayPoint(){
    emit WPValuesChanged(this);
    this->map->update();
}

int WayPointItem::snumber=1;

void WayPointItem::NewMouseClick(){
    if (m_mouseDown)
    {
        //Mouse is already pressed
        return;
 ...
 void WayPointItem::RecieveNums(double lng, const double lat, const double alt){
    coord.SetLng(lng);
    coord.SetLat(lat);
    altitude = alt;
    emit WPValuesChanged(this);
