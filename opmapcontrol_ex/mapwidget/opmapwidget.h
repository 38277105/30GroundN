...
public:
    ...
    WayPointItem* WPCreateBoundary(internals::PointLatLng const& coord, double const& altitude, MapGraphicItem* map, const WayPointType& type, const bool& isShowNum);
...
private:
    ...
    ZoomChangeItem*    zoomUp;
    ZoomChangeItem*    zoomDown;
    bool useOpenGL;
...
private slots:
    void diagRefresh();
    void zoomChangeItem_slot(ZoomChangeType type);
...
public slots:
   ...
   void Offset(int const& x, int const& y){map->Offset(x,y);}

   void resizeZoomChangeIcon(const QRectF &rect);
};
