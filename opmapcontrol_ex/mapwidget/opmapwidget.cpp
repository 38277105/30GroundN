    ...
    diagGraphItem(0)
{
    ...
    SetShowCompass(true);
    zoomUp = new ZoomChangeItem();
    zoomUp->setType(ZOOMUP);
    zoomUp->setPixmap(QString::fromUtf8(":/markers/images/up.png"));
    connect(zoomUp, SIGNAL(signal_zoomChang(ZoomChangeType)), this, SLOT(zoomChangeItem_slot(ZoomChangeType)));
    mscene.addItem(zoomUp);
    //QRectF sf = mscene.sceneRect();

   // zoomUp->setPos(sf.x(), sf.width()*2);

    zoomDown = new ZoomChangeItem();
    zoomDown->setType(ZOOMDOWN);
    zoomDown->setPixmap(QString::fromUtf8(":/markers/images/down.png"));
    connect(zoomDown, SIGNAL(signal_zoomChang(ZoomChangeType)), this, SLOT(zoomChangeItem_slot(ZoomChangeType)));
    mscene.addItem(zoomDown);
    //zoomDown->setPos(sf.x(), sf.height()+150);
    connect(map,SIGNAL(siganl_BoundingRectChanged(QRectF)),this,SLOT(resizeZoomChangeIcon(QRectF)));
}
...
void OPMapWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    emit OnCurrentPositionChanged(currentmouseposition);
...
WayPointItem* OPMapWidget::WPCreateBoundary(internals::PointLatLng const& coord, double const& altitude, MapGraphicItem* map, const WayPointType& type, const bool& isShowNum){
    WayPointItem* item=new WayPointItem(coord, altitude, map, type, isShowNum);
    ConnectWP(item);
    item->setParentItem(map);
    return item;
}
...
void OPMapWidget::zoomChangeItem_slot(ZoomChangeType type){
    qDebug()<<"real Zoom"<<ZoomReal();
    if(type == ZOOMUP){
        this->SetZoom(this->ZoomReal()+1);
    }else if(type == ZOOMDOWN){
        this->SetZoom(this->ZoomReal()-1);
    }
}
...
void OPMapWidget::resizeZoomChangeIcon(const QRectF &rect){
    zoomUp->setPos(rect.width()-50, rect.height()-150);
    zoomDown->setPos(rect.width()-50, rect.height()-70);
}
